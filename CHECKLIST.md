...
- [x] Auth Phase 4: API keys — `ApiKey` entity + argon2id (`@node-rs/argon2`) + `IssueApiKey`/`RevokeApiKey` + `getBusinessAuthContext` real (header `Authorization: Bearer sg_…`, scope `read`/`write`). Migración `0004` aplicada. Pendiente: panel de gestión de keys en el dashboard (CU-07)
