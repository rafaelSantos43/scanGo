...
- [x] `apps/pwa` — pantalla `/scan` con `@yudiel/react-qr-scanner`, mensajes específicos por código de error del backend. Auth via cookies HttpOnly (Phase 3), `scanQrToken(token)` ya no recibe IDs, fetch con `credentials: 'include'`.
- [x] `apps/pwa` — landing `/` consume `/api/v1/me/customer` para saber quién está logueado. Sin sesión, pinta "abre tu enlace de invitación"; con sesión, "Hola {fullName}" + botón a `/scan`. Stub de localStorage eliminado (D-027 wires the cookie real).
...
- [x] Auth Phase 3: customer magic link. Use cases `RequestCustomerMagicLink`/`VerifyCustomerMagicLink`, callback `/api/auth/customer/callback?customer_id=...&token_hash=...`, tres cookies HttpOnly (`sg_customer_session` + `sg_customer_refresh` + `sg_customer_id`), `getCustomerAuthContext` async real (valida JWT + verifica `customers.user_id === verifiedUserId`), proxy refresh de las cookies del customer. `CreateCustomerAction` envía invite automático tras crear (CU-02 paso 3); falla blanda. Script `dev:customer-link <customer_id>` para testing sin email. Repo gana `findByIdAcrossBusinesses` (cross-tenant, mismo patrón que ApiKey.findByPrefix) y `linkUserId` (idempotente, WHERE user_id IS NULL).
