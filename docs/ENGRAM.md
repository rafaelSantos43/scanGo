...
**Fecha:** 2026-05-14
**Cambio:** v1.4 — Arranca M1. Capa Domain del flujo de escaneo modelada por `agent_data` (sub-agente): 5 entidades (Business, Customer, Package, QrToken, Attendance), 6 IDs branded, 4 VOs (Email, VisitCount, Timezone, Slug), 14 errores con `code` único, 5 interfaces de repositorio (todas filtran por `businessId` salvo `findBySlug` que es público), 2 interfaces de servicio (Clock, IdGenerator). 58 tests unitarios verdes en 21ms. Cero imports externos en `domain/` (Clean Architecture §3.1 respetada). Pendiente: entidades de webhooks/ApiKey/BusinessAdmin, schemas Zod, migraciones, repos Drizzle, use cases.
