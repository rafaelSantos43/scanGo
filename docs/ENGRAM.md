...
| 3.7 | 2026-05-20 | PWA instalable (RNF-04): `manifest.ts`, `icon.tsx`/`icon2.tsx`/`apple-icon.tsx` con `ImageResponse`, SW mínimo en `public/sw.js` + `ServiceWorkerRegister` (solo prod por default), `metadata` y `viewport` reales en `layout.tsx`. Sin librería externa — Next 16 nativo cubre todo. 78 tests verdes. |
