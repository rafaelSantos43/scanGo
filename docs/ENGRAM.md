...
**Fecha:** 2026-05-14
**Cambio:** v1.5 — Segundo chunk de M1 entregado por `agent_data`: capa Infrastructure del flujo de escaneo. Schema Drizzle (5 tablas: businesses, customers, packages, qr_tokens, attendances) con FKs, unique constraints (incluyendo partial `one_active_package_per_customer WHERE status='active'`). Migración inicial `0000_init_scan_flow.sql` con políticas RLS para las 5 tablas (D-007). 5 mappers explícitos `toDomain/toPersistence`. 5 repositorios Drizzle implementando las interfaces del dominio (todos filtran por businessId; `AttendanceRepositoryDrizzle.save` mapea PG error 23505 + constraint a `AlreadyScannedTodayError`). `composition.ts` con factories planas y singleton de Drizzle client lazy-inicializado desde `DATABASE_URL`. Cero imports prohibidos. Hallazgo: el agente identificó que los métodos atómicos (`claim` del QR, `decrementVisitAtomic` del Package) requieren extender los repos en el chunk del use case `RegisterAttendance` — registrado en CHECKLIST.
